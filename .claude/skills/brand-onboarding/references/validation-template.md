...
- [ ] A demo video → backend/content/videos/demo.mp4 + a VIDEOS entry in
      seed_data.py (files without an entry are ignored at seed time)
...
3. Apply each answer immediately, per surface-map.md:
   ...
   - Supplied assets → copy into place (public/brand/, backend/content/…).
     A demo video also needs a `VIDEOS` entry in seed_data.py (_id, title,
     description, file) — without it the file is ignored at seed time; then
     let the reload reseed and re-run smoke.py.
   - Real slide exports replace a deck dir's PNGs wholesale (delete
     placeholders first; sortable names `01.png, 02.png, …`). If the slide
     count changed, update that deck's presenter_notes AND the generator's
     `DECKS` outline to the same count — the generator asserts they match.
     And once any real exports are in place, do not re-run the generator (it
     regenerates every deck as placeholders); make further note/outline edits
     directly in the two files instead.
   - A removed proof line → delete it from the allowlist, then run an exact
     search for the name/number/quote across every generated surface —
     `grep -rFi -- "<term>" backend/app/orchestrator/prompt.py backend/app/seed_data.py`
     (GTM sections, deck titles/descriptions, presenter notes, greeting,
     topics, persona description) — and remove every occurrence before
     marking the item resolved; the system prompt exposes all of them.
...
apply the delta per surface-map.md and re-run the Phase 4 checks. In
VALIDATION.md, first flip any previously-resolved item the correction
supersedes from `[x]` to `[s]` (append a short dated note of what replaced
it), then add the new item and work it — a stale `[x]` approval must never
remain the apparent current state. Append; don't regenerate the file.
